/**
 * Markdown-aware text wrapping.
 * Preserves code blocks, tables, headers, etc. verbatim.
 */

export function wrapLine(text: string, maxLen: number, indent: string = ''): string {
  ...
  for (const line of lines) {
    ...
    if (fenceMatch) {
      if (!inFence) {
        ...
        result.push(line)
        continue
      } else if (fencePattern && line.startsWith(fencePattern) && line.trim() === fencePattern) {
        inFence = false
        fencePattern = null
        result.push(line)
        continue
    ...
    if (/^( {4,}|\t)/.test(line)) { result.push(line); continue }

    // Header
    if (/^#{1,6}\s/.test(line)) { result.push(line); continue }

    // Table row (starts with |)
    if (/^\|/.test(line.trimStart())) { result.push(line); continue }

    // Horizontal rule
    if (/^[-*_]{3,}\s*$/.test(line)) { result.push(line); continue }
